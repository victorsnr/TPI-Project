...
public class TelaCriarExposicao extends JPanel{
	private IArtGallery minhaGaleria;
	private DefaultListModel<Obra> listModelObras;

	public TelaCriarExposicao (CardLayout cards, JPanel painelPrincipal, IArtGallery minhaGaleria) {
			...
			if (!nomeExpo.isBlank()) {
				...
				if (!obrasSelecionadas.isEmpty()) {
					for (Obra obra : obrasSelecionadas) {
						newExpo.adicionarObra(obra);
					}
				}
				
				try {
					minhaGaleria.adicionarExposicao(newExpo);
					JOptionPane.showMessageDialog(this, "Exposição Criada com sucesso", "ArtGallery diz:", JOptionPane.INFORMATION_MESSAGE);
					textFieldNomeExpo.setText("");
					cards.show(painelPrincipal, "INICIO");
				} catch (ExposicaoExistenteException erro){
